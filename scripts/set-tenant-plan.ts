...
if (!TENANT_SLUG) {
  ...
  console.error('Available tiers: starter, pro, business, enterprise');
  process.exit(1);
}
...
async function setTenantPlan() {
  try {
    ...
    const plan = await prisma.subscriptionPlan.findFirst({
      where: { tier: PLAN_TIER as never, isActive: true },
    });
    if (!plan) {
    ...
    // Check for existing subscription (Subscription.tenantId is @unique)
