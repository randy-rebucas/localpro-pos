#!/usr/bin/env tsx
/**
 * 1POS — MongoDB -> PostgreSQL ETL (Phase 3 of the Postgres migration plan)
 *
 * Reads every Mongoose collection and writes the normalized rows into the
 * new Prisma/Postgres schema (see prisma/schema.prisma). One-time, idempotent
 * (uses Mongo ObjectId hex strings as the Postgres PK/FK values directly, so
 * re-running with `createMany({ skipDuplicates: true })` is safe).
 *
 * Usage:
 *   npx tsx scripts/migrate-to-postgres.ts [options]
 *   npm run db:migrate-to-postgres -- [options]
 *
 * Options:
 *   --dry-run        Read Mongo only, print counts per target table, write nothing.
 *   --only=<model>   Run a single collection's migration in isolation (Mongoose
 *                     model name, case-insensitive, e.g. --only=Product).
 *
 * Exit codes:
 *   0  All entity groups migrated without failure (or dry-run completed)
...
import PosTable from '../models/Table';
...
async function main() {
  ...
  if (failures.length === 0) {
    console.log(DRY_RUN ? 'Dry-run completed with no errors.' : 'Migration completed with no errors.');
  } else {
