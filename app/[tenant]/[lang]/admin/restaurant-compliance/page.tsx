...
export default function RestaurantCompliancePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch { toast.error(dict?.admin?.failedToLoadRestaurantCompliance || 'Failed to load restaurant compliance'); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) toast.success(dict?.admin?.restaurantComplianceSaved || 'Restaurant compliance settings saved');
    ...
    <div className="px-4 sm:px-6 py-6">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{dict?.admin?.restaurantComplianceTitle || 'Restaurant / Food Service Compliance'}</h1>
            <p className="text-sm text-gray-500 mt-0.5">{dict?.admin?.restaurantComplianceSubtitle || 'RA 10611 Food Safety Act — FDA, DOH, and LGU requirements'}</p>
          </div>
        </div>
          ...
          <aside className="w-52 shrink-0 sticky top-6">
            <div className="bg-white border border-gray-300 p-4">
              ...
              <ul className="space-y-2 text-xs text-gray-600">
                <li className="flex gap-2"><span className="text-brand font-bold mt-0.5">·</span>{dict?.admin?.rcFdaLicense || 'FDA Food Business License (RA 10611)'}</li>
                <li className="flex gap-2"><span className="text-brand font-bold mt-0.5">·</span>{dict?.admin?.rcFoodSafetyCert || 'Food Safety Certificate'}</li>
                <li className="flex gap-2"><span className="text-brand font-bold mt-0.5">·</span>{dict?.admin?.rcFoodHandlersCert || 'Food Handlers Health Certificates'}</li>
                <li className="flex gap-2"><span className="text-brand font-bold mt-0.5">·</span>{dict?.admin?.rcKitchenSanitation || 'Kitchen Sanitation Compliance'}</li>
              </ul>
              <p className="text-xs text-gray-400 mt-4">{dict?.admin?.rcExpiryNote || 'Set expiry dates to receive advance warnings before documents lapse.'}</p>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="bg-white border border-gray-300">
              <div className="px-5 py-3 border-b border-gray-200 bg-gray-50">
                <h2 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">{dict?.admin?.fdaFblTitle || 'FDA Food Business License (FBL)'}</h2>
                <p className="text-xs text-gray-400 mt-0.5">{dict?.admin?.fdaFblDesc || 'Required for all food businesses under RA 10611. Apply at the nearest FDA office.'}</p>
              </div>
              <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.fblNumber || 'FBL Number'}</label>
                  <input type="text" value={data.fdaFoodBusinessLicense ?? ''} onChange={e => set('fdaFoodBusinessLicense')(e.target.value)} placeholder={dict?.admin?.fblNumberPlaceholder || 'e.g. FBL-XXXXXXXX'}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.fblExpiryDate || 'FBL Expiry Date'}</label>
            ...
            <div className="bg-white border border-gray-300">
              <div className="px-5 py-3 border-b border-gray-200 bg-gray-50">
                <h2 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">{dict?.admin?.foodSafetyCertTitle || 'Food Safety Certificate'}</h2>
                <p className="text-xs text-gray-400 mt-0.5">{dict?.admin?.foodSafetyCertDesc || 'Certificate of compliance with food safety management system requirements.'}</p>
              </div>
              <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.certificateNumber || 'Certificate Number'}</label>
                  <input type="text" value={data.foodSafetyCertificateNumber ?? ''} onChange={e => set('foodSafetyCertificateNumber')(e.target.value)}
                    className="w-full border border-gray-300 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.certificateExpiry || 'Certificate Expiry'}</label>
            ...
            <div className="bg-white border border-gray-300">
              <div className="px-5 py-3 border-b border-gray-200 bg-gray-50">
                <h2 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">{dict?.admin?.foodHandlersTitle || 'Food Handlers'}</h2>
                <p className="text-xs text-gray-400 mt-0.5">{dict?.admin?.foodHandlersDesc || 'All food handlers must hold valid health certificates from the LGU Health Office.'}</p>
              </div>
              <div className="p-5 space-y-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700">{dict?.admin?.allFoodHandlersCertified || 'All food handlers have valid health certificates'}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700">{dict?.admin?.kitchenSanitationMet || 'Kitchen sanitation standards are met (RA 10611)'}</span>
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-gray-100">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.numberOfCertifiedHandlers || 'Number of Certified Food Handlers'}</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">{dict?.admin?.earliestHealthCertExpiry || 'Earliest Health Certificate Expiry'}</label>
