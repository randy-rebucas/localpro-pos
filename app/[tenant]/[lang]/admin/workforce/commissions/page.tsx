      ...
      <div className="px-6 py-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2 items-center">
            <input type="month" value={period} onChange={e => setPeriod(e.target.value)} className="border border-gray-300 px-2 py-1 text-sm" />
            <button onClick={calculate} disabled={loading} className="px-3 py-1.5 text-sm text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50">
              Recalculate
            </button>
          </div>
        </div>

        {error && <div className="mb-3 p-2 bg-red-50 border border-red-200 text-red-700 text-sm">{error}</div>}
        ...
        {/* Bulk actions */}
        {selected.size > 0 && (
          <div className="mb-3 p-2 bg-blue-50 border border-blue-200 flex items-center gap-3 text-sm">
              ...
              {loading ? (
                ...
                <tr key={c._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-center">
                    <span className={`px-2 py-0.5 text-xs font-medium ${STATUS_BADGE[c.status]}`}>{c.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
