    ...
    try {
      ...
      if (res.ok) {
        ...
        if (retryCount > POLLING_CONFIG.RETRY_LIMIT) {
          setError('Session not found. Please open this from the POS terminal.');
    ...
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800" role="alert" aria-label="No session found error">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-white mb-4">No Session Found</h1>
          <p className="text-gray-400">Please open this page with a valid session ID</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800" role="status" aria-label="Waiting for items to be scanned">
        <div className="text-center">
          ...
          <h1 className="text-4xl font-bold text-white mb-4">Waiting for Items...</h1>
          <p className="text-gray-400 text-xl">Scan items to get started</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-gradient-to-br from-green-600 to-green-900" role="status" aria-label="Payment completed successfully">
        <div className="text-center">
          ...
          <p className="text-green-100 text-2xl">Payment Complete</p>
          <p className="text-green-200 text-xl mt-4">Total: <Currency amount={sessionData.total} /></p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-screen h-screen bg-gradient-to-br from-gray-900 to-gray-800 flex flex-col p-8 overflow-hidden">
      ...
      <div className="text-center mb-8 flex-shrink-0">
        <h1 className="text-6xl font-bold text-white mb-2">Your Order</h1>
        <p className="text-gray-400 text-xl" role="status" aria-label={`Session ID: ${sessionId.substring(0, 8)}`}>Session: {sessionId.substring(0, 8)}...</p>
      </div>
      ...
      <div className="flex-1 flex gap-8 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="bg-gray-800 rounded-2xl p-8 border border-gray-700 flex-1 overflow-y-auto overscroll-contain">
            <h2 className="text-3xl font-bold text-white mb-6 flex-shrink-0">Items</h2>
              ...
              {sessionData.cart.map((item) => (
                <div key={item.productId} className="bg-gray-700 rounded-xl p-6 flex justify-between items-center flex-shrink-0">
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <p className="text-gray-400 text-lg">Qty: {item.quantity}</p>
                  </div>
          ...
          <div className="bg-gray-700 rounded-xl p-6 mt-4 border border-gray-600 flex-shrink-0">
            ...
            {sessionData.taxAmount !== undefined && sessionData.taxAmount > 0 && (
              <div className="flex justify-between items-center mb-4 text-amber-300">
                <p className="text-lg">{sessionData.taxLabel || 'Tax'} {sessionData.taxRate ? `(${sessionData.taxRate}%)` : ''}</p>
                <p className="text-2xl font-bold">
            ...
            <div className="border-t border-gray-600 pt-4">
              <div className="flex justify-between items-center">
                <p className="text-gray-300 text-2xl font-semibold">Total</p>
          ...
          {showPaymentScreen && (
            <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-8 border border-blue-500 flex flex-col justify-between flex-1 min-h-0 overflow-y-auto">
              <div className="flex-shrink-0">
                <h2 className="text-4xl font-bold text-white mb-2">Ready to Pay?</h2>
                <p className="text-blue-200 text-lg">Total: <Currency amount={sessionData.total} className="text-4xl" /></p>
              </div>
              ...
              <p className="text-center text-blue-200 text-lg flex-shrink-0">Staff will complete payment</p>
            </div>
          )}
