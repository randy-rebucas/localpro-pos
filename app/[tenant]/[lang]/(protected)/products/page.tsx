...
export default function ProductsPage() {
  ...
  const hasInitiatedEditRef = useRef(false);
  ...
  const businessType = getBusinessType(settings ?? undefined);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch(`/api/categories?tenant=${tenant}`);
  ...
  useEffect(() => {
    // Only attempt to open edit modal if we have an edit ID and haven't already initiated
    if (editProductId && !hasInitiatedEditRef.current && dict) {
      hasInitiatedEditRef.current = true;
      
      // Directly fetch the product from the API
      fetch(`/api/products/${editProductId}?tenant=${tenant}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  // Sort products: pinned first (if property exists), then by creation date
  const sortedProducts = [...products].sort((a, b) => {
    const aPinned = (a as any).pinned; // eslint-disable-line @typescript-eslint/no-explicit-any
    const bPinned = (b as any).pinned; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const handleCloseModal = () => {
    ...
    hasInitiatedEditRef.current = false;
    fetchProducts();
  };
