...
export default function Dashboard() {
  ...
  }, [showAgeVerificationModal, showSplitCheckModal, showPaymentModal, showRefundModal, showSavedCartsModal, showSaveCartModal, showQRScanner, showCashDrawerModal, closingSummary]);
    ...
    <div className="min-h-screen">
          ...
          <div className={`order-2 lg:order-1 ${roamingMode ? 'w-full' : 'lg:col-span-2'}`}>
            <div className="mb-6 sm:mb-8">
              <div className="flex items-center justify-between mb-4 sm:mb-6 gap-4">
                    ...
                    <div className="flex gap-1">
                      {(['dine-in', 'takeout', 'delivery'] as const).map((type) => (
                        <button
                          ...
                          className={`px-3 py-2 text-sm font-semibold border-2 transition-all capitalize ${
                            orderType === type
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 text-sm font-medium border transition-colors flex-shrink-0 ${
                    roamingMode
                      ? 'text-white border-transparent'
            ...
            {!isCashier && (
              <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
                ...
                {/* Restaurant: Order # chip instead of Split Check */}
                {businessType === 'restaurant' ? (
                  <div className="flex-shrink-0 flex flex-col justify-center gap-1 px-4 py-2 min-h-[64px] bg-orange-50 border-2 border-orange-200 text-gray-700 font-medium text-xs min-w-[120px]">
                    <span className="text-[10px] font-semibold text-orange-600 uppercase tracking-wide">
                      {orderType === 'dine-in' ? 'Table #' : 'Order #'}
                    </span>
                    <input
                      type="text"
                      value={tableNumber}
                      onChange={(e) => setTableNumber(e.target.value.slice(0, 6))}
                      placeholder={orderType === 'dine-in' ? '5A' : 'T/O-1'}
                      className="w-full px-2 py-1 text-sm border border-orange-300 bg-white font-semibold focus:outline-none focus:border-orange-500"
                    />
                  </div>
                ) : (
                  ...
                  <button
                    type="button"
                    onClick={() => { if (cart.length > 0) setShowSplitCheckModal(true); else showToast.error(dictValue('pos.cartEmptyAlert', 'Cart is empty')); }}
                    className="flex-shrink-0 flex flex-col items-center gap-1.5 px-5 py-3 min-h-[64px] bg-white border-2 border-gray-200 hover:border-gray-400 hover:bg-gray-50 transition-all text-gray-700 font-medium text-xs"
                ...
                {(businessType === 'service' || businessType === 'laundry') && (
                  <div className="flex-shrink-0 flex flex-col justify-center gap-1 px-4 py-2 min-h-[64px] bg-blue-50 border-2 border-blue-200 text-gray-700 font-medium text-xs min-w-[140px]">
                    <span className="text-[10px] font-semibold text-blue-600 uppercase tracking-wide">Schedule for</span>
                    <input
                      type="datetime-local"
                      value={scheduledFor}
                      onChange={(e) => setScheduledFor(e.target.value)}
                      className="w-full px-1 py-1 text-xs border border-blue-300 bg-white font-medium focus:outline-none focus:border-blue-500"
                    />
                  </div>
                )}
                ...
                <button
                  type="button"
                  onClick={() => setShowRefundModal(true)}
                  className="flex-shrink-0 flex flex-col items-center gap-1.5 px-5 py-3 min-h-[64px] bg-white border-2 border-gray-200 hover:border-red-200 hover:bg-red-50 transition-all text-gray-700 font-medium text-xs"
                ...
                {[...products]
                  ...
                  >
                    ...
                    {businessType !== 'restaurant' && businessType !== 'service' && businessType !== 'laundry' && availableStock <= 10 && (
                      <div className={`absolute top-2 left-2 z-20 min-w-[1.5rem] h-6 px-1.5 flex items-center justify-center text-[11px] font-bold text-white shadow-md ${
                        availableStock === 0
                          ? 'bg-red-500'
                          : 'bg-yellow-500'
                      }`}>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.productId} className="flex items-center gap-4 bg-gray-50 border border-gray-200 p-3">
                  ...
                  {([
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 border-2 transition-all duration-150 flex flex-col items-center justify-center gap-1.5 ${
                          isSelected
                            ? 'border-2'
                  ...
                  <div className="flex gap-2">
                    {(['Visa', 'Mastercard', 'Amex', 'Other'] as const).map((brand) => (
                      <button
                        ...
                        className={`flex-1 py-2 text-xs font-semibold border-2 transition-all ${
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {(['GCash', 'Maya', 'PayPal', 'ShopeePay', 'Grab', 'Other'] as const).map((w) => (
                          <button
                            ...
                            className={`py-2 px-1 text-xs font-semibold border-2 transition-all ${
                              paymentProvider === w
                                ? 'text-white border-transparent'
                                : 'bg-gray-50 border-gray-200 text-gray-600 hover:border-gray-300'
                            }`}
      ...
      {showSplitCheckModal && (
        <div
          ...
          onClick={() => setShowSplitCheckModal(false)}
        >
          <div
          ...
          >
            ...
            <div className="px-6 py-5">
              ...
              <button
                type="button"
                onClick={() => {
                  showToast.success(`Bill split ${splitGuests} ways — each guest owes `);
                  setShowSplitCheckModal(false);
                }}
