    ...
    for (const tenant of tenants) {
      ...
      try {
        ...
        if (archive) {
          ...
          // Insert archive batch (skipDuplicates continues on partial failures,
          // mirroring the previous Mongo insertMany({ ordered: false }) behavior)
          await prisma.archivedAuditLog.createMany({ data: archiveDocs, skipDuplicates: true });
        }

        // Delete the expired logs from the live table
        const ids = expiredLogs.map(l => l.id);
        const deleteResult = await prisma.auditLog.deleteMany({ where: { id: { in: ids } } });
        results.processed += deleteResult.count ?? 0;
      } catch (error: unknown) {
