...
export async function processSubscriptionBilling(
  ...
  const planMap = new Map<string, PrismaSubscriptionPlan>(plans.map((p) => [p.id, p]));
  ...
  try {
    ...
    for (const sub of toDeactivate) {
      try {
        ...
        // Tenant deactivation is scoped to this subscription's own tenantId only.
        await prisma.tenant.update({ where: { id: sub.tenantId }, data: { isActive: false } });
      ...
      try {
        ...
        try {
          ...
          await notifyAdmin(
            ...
            `Tenant: ${tenant?.name || sub.tenantId} (${sub.tenantId})\nSubscription: ${sub.id}\nLate fee: ${lateFeeAmount} ${plan.priceCurrency || 'PHP'}\nOutstanding balance: ${newOutstandingBalance} ${plan.priceCurrency || 'PHP'}`
          );
        } catch (adminError) {
          const adminMsg = adminError instanceof Error ? adminError.message : String(adminError);
    ...
    for (const sub of toReactivationFee) {
      try {
        ...
        await prisma.subscription.update({
          where: { id: sub.id },
          data: { outstandingBalance: newOutstandingBalance, reactivationFeeAppliedAt: now },
        });
        ...
        try {
          ...
          await notifyAdmin(
            ...
            `Tenant: ${tenant?.name || sub.tenantId} (${sub.tenantId})\nSubscription: ${sub.id}\nReactivation fee: ${fee} ${plan.priceCurrency || 'PHP'}\nOutstanding balance: ${newOutstandingBalance} ${plan.priceCurrency || 'PHP'}`
          );
        } catch (adminError) {
          const adminMsg = adminError instanceof Error ? adminError.message : String(adminError);
