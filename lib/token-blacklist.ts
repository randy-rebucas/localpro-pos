 ...
 *
 * Uses Postgres as the persistent store for multi-instance / serverless compatibility.
 * Falls back to in-memory store if Postgres is not available during the check.
 *
 * Tokens are stored by their SHA-256 hash until their natural expiration.
 * Postgres has no native TTL index — expired rows need a scheduled cleanup job
 * (same pattern as AuditLog/PosSession; see prisma/schema.prisma's RevokedToken model).
 */
...
export async function revokeToken(
  ...
  try {
    await prisma.revokedToken.upsert({
      where: { tokenHash: key },
      create: { id: randomUUID(), tokenHash: key, reason, expiresAt },
...
export async function revokeAllUserTokens(userId: string): Promise<void> {
  ...
  try {
    await prisma.userRevocation.upsert({
      where: { userId },
      create: { id: randomUUID(), userId, revokedBefore: new Date(now) },
