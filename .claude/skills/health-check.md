...
4. If a table is missing: check if the model/count query is present in `scripts/health-check.ts`. Add it if needed.
...
| `pnpm run reset:collections` | **DESTRUCTIVE** — deletes all rows from every table |
