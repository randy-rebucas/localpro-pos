...
export default function Navbar() {
  ...
  const conditionalNavItems: Array<{ href: string; key: string; featureFlag?: string }> = [
    ...
    { href: `/${tenant}/${lang}/admin/crm`, key: 'crm', featureFlag: 'enableCustomerManagement' },
  ];
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="text-2xl font-bold" style={{ color: primaryColor }}>POS</div>
          </div>
        </div>
      </nav>
    );
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="inline-flex items-center justify-center p-2 border border-gray-300 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-inset transition-colors bg-white"
                style={{ outlineColor: primaryColor }}
                onFocus={(e) => { e.currentTarget.style.boxShadow = `0 0 0 2px ${primaryColor}30`; e.currentTarget.style.borderColor = primaryColor; }}
              ...
              {/* User Manual — top navbar, left side */}
              <Link
                href={`/${tenant}/${lang}/user-manual`}
                className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border transition-colors ${
                  pathname.includes('/user-manual')
                    ? 'text-white border-transparent'
                    : 'text-gray-600 border-gray-300 hover:bg-gray-50 bg-white'
                }`}
                style={pathname.includes('/user-manual') ? { backgroundColor: primaryColor, borderColor: primaryColor } : {}}
              >
                <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
                <span>{dict?.nav?.userManual || 'User Manual'}</span>
              </Link>
            </div>
                  ...
                  <div className="absolute right-0 mt-2 w-56 border border-gray-300 bg-white z-50">
                    <div className="py-1">
                      ...
                      <Link
                        ...
                        style={pathname.includes('/profile') ? { backgroundColor: `${primaryColor}10`, color: primaryColor } : {}}
                      >
                      ...
                      {isAdmin && (
                        <Link
                          ...
                          onClick={() => setUserMenuOpen(false)}
                          className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                          style={pathname.includes('/admin') ? { backgroundColor: `${primaryColor}10`, color: primaryColor } : {}}
                        >
                          <svg className="w-5 h-5 mr-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <Link
                        ...
                        onClick={() => setUserMenuOpen(false)}
                        className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                        style={pathname.includes('/settings') ? { backgroundColor: `${primaryColor}10`, color: primaryColor } : {}}
                      >
                        <svg className="w-5 h-5 mr-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <>
                ...
                <div className="p-2 space-y-1">
                  <Link
                    ...
                    style={pathname.includes('/profile') ? { backgroundColor: `${primaryColor}15`, color: primaryColor, borderColor: primaryColor } : { backgroundColor: '#fff' }}
                  >
                  ...
                  {isAdmin && (
                    <Link
                      ...
                      onClick={() => setDrawerOpen(false)}
                      className="flex items-center px-4 py-3 border border-gray-300 text-base font-medium transition-colors"
                      style={pathname.includes('/admin') ? { backgroundColor: `${primaryColor}15`, color: primaryColor, borderColor: primaryColor } : { backgroundColor: '#fff' }}
                    >
                      <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                      </svg>
                  ...
                  <Link
                    ...
                    onClick={() => setDrawerOpen(false)}
                    className="flex items-center px-4 py-3 border border-gray-300 text-base font-medium transition-colors"
                    style={pathname.includes('/settings') ? { backgroundColor: `${primaryColor}15`, color: primaryColor, borderColor: primaryColor } : { backgroundColor: '#fff' }}
                  >
                    <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    ...
                    <div className="p-2">
                      <div className="border p-3" style={{ borderColor: primaryColor, background: `${primaryColor}08` }}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          style={subscriptionStatus.isTrial && !subscriptionStatus.isTrialExpired ? { backgroundColor: `${primaryColor}15`, color: primaryColor } : undefined}
                          >
                            {subscriptionStatus.isActive
